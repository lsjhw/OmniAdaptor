    ...
    def _get_arguments(self):
        ...
        if os.path.isfile(self.args.input_data):
            ...
            if not self.FILE_PATTERN.match(filename_lower):
                raise ValueError(f"Provided file invalid: {os.path.basename(self.args.input_data)}")
        elif os.path.isdir(self.args.input_data):
            self.input_is_file = False
        else:
            raise ValueError(f"Input path is neither a file nor a directory: {self.args.input_data}")
